extern "C" {
    ...
    __global__ void CudaBuildStep(
        ...
        const int numCols,
        const int numFeatures, // for record-keeping purposes. May change later. 
        const int numRows,
        ...
        if (kAgentId < (kNumAgents - 1)) { 
            ...
            if (actions[agentSpecificPointer] == 1) {
                if (AgentCanBuild(
                    ...
                    int numEnvs = sizeof(env_timestep_arr);

                    int recordIndex = (numTimesteps * (kNumAgents - 1) * numFeatures) * kEnvId 
                                    + ((kNumAgents - 1) * numFeatures) * env_timestep_arr[kEnvId]
                }
            }
        }
    }
