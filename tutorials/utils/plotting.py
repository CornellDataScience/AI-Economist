...
def get_rewards_from_log(log):
    csv_string = ""
    if "rewards" in log:
        for t, rwds in enumerate(log["rewards"]):
            if isinstance(rwds, dict):
                csv_string = csv_string + str(rwds["0"]) + "," + str(rwds["1"]) + "," + str(rwds["2"]) + "," + str(rwds["3"]) + "," + str(rwds["p"]) + "\n"
    return csv_string

def get_all_rwds_from_all_logs(dense_log_dir):
    csv_file = os.path.join(dense_log_dir, "rwds.csv")
    csv_string = ""
    for root, dirs, files in os.walk(dense_log_dir):
        for log_file in files:
            if "csv" not in log_file:
                file_path = os.path.join(root, log_file)
                dense_log = load_episode_log(file_path)
                csv_string = csv_string + get_rewards_from_log(dense_log)
    with open(csv_file,'w') as file:
        file.write(csv_string)
